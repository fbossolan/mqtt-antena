...
from mqtt_manager import add_client, get_client, remove_client, listeners, listeners_lock
import json
import time
...
app.config['SQLALCHEMY_DATABASE_URI'] = f'sqlite:///{os.path.join(data_dir, "antena.db")}'
...
def brokers():
    if request.method == 'POST':
        if 'add' in request.form:
            ...
            new_broker = Broker(name=name, ip=ip, port=port, username=user, password=password)
...
def toggle_listen():
    ...
    if client:
        if action == 'stop':
            ...
            flash(f'Listening to {topic if topic else "all topics"} on {client.name}', 'success')
